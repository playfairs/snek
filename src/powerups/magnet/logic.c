...
static int positions_equal(const Position *a,
                           const Position *b)
{
    return a->x == b->x && a->y == b->y;
}

static int is_near_head(const Position *head,
...
static void move_toward_head(Position *target,
                             const Position *head)
{
    if (target->x < head->x)
        target->x += SNAKE_BLOCK;
    else if (target->x > head->x)
        target->x -= SNAKE_BLOCK;

    if (target->y < head->y)
        target->y += SNAKE_BLOCK;
...
static void clamp_position(Position *pos)
{
    if (pos->x < 0)
        pos->x = 0;
    if (pos->x >= DIS_WIDTH)
        pos->x = DIS_WIDTH - SNAKE_BLOCK;
    if (pos->y < GAME_AREA_TOP)
        pos->y = GAME_AREA_TOP;
    if (pos->y >= GAME_AREA_BOTTOM)
        pos->y = GAME_AREA_BOTTOM - SNAKE_BLOCK;
}
...
void magnet_update(GameContext *game)
{
    ...
    if (game->food.active
        && is_near_head(&head, &game->food.pos, range))
    {
    ...
    if (game->powerup.active
        && is_near_head(&head, &game->powerup.pos, range))
    {
