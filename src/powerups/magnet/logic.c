...
static int is_near_head(const Position *head, const Position *target,
                        int range) {
...
void magnet_apply(GameContext *game) { (void)game; }
...
void magnet_update(GameContext *game) {
  ...
  if (game->secondary_food.active && game->secondary_food.type != ITEM_BOMB &&
      is_near_head(&head, &game->secondary_food.pos, range)) {
...
void magnet_remove(GameContext *game) { (void)game; }
