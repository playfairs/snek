...
{
    return type == POWERUP_PATHFIND
           || type == POWERUP_DOUBLE_POINTS
           || type == POWERUP_FRENZY;
}
...
static Position wrap_position(int x, int y)
{
    ...
    int wrapped_y =
        wrap_value((y - GAME_AREA_TOP) / SNAKE_BLOCK,
                   grid_height);
...
static int
is_safe_move(GameContext *game, int new_dx, int new_dy)
{
    Position head =
        game->snake.segments[game->snake.length - 1];
    Position new_pos =
        wrap_position(head.x + new_dx, head.y + new_dy);
    int invincible =
    ...
    if (!invincible)
    {
        ...
        {
            if (positions_equal(game->snake.segments[i],
                                new_pos))
            {
                return 0;
            }
    ...
    if (!invincible && game->secondary_food.active
        && game->secondary_food.type == ITEM_BOMB)
    {
        if (positions_equal(new_pos,
                            game->secondary_food.pos))
        {
            return 0;
        }
    }
...
static int
is_opposite_direction(int dx1, int dy1, int dx2, int dy2)
{
    return dx1 == -dx2 && dy1 == -dy2;
}

static int select_target(GameContext *game,
                         Position *target_pos)
{
    ...
    if (game->powerup.active
        && is_powerup_target_type(game->powerup.type))
    {
        ...
        int head_distance = grid_distance_sq(
            game->snake.segments[game->snake.length - 1],
            powerup_pos);
        int food_distance = grid_distance_sq(
            game->snake.segments[game->snake.length - 1],
            food_pos);
...
void pathfind_update_intelligence(GameContext *game)
{
    ...
    int current_safe =
        is_safe_move(game, current_dx, current_dy);
    int current_dist = current_safe
                           ? grid_distance_sq(head, target)
                           : INT_MAX;
    ...
    int directions[4][2] = {{0, -SNAKE_BLOCK},
                            {0, SNAKE_BLOCK},
                            {-SNAKE_BLOCK, 0},
                            {SNAKE_BLOCK, 0}};
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (is_opposite_direction(new_dx,
                                  new_dy,
                                  current_dx,
                                  current_dy))
        {
