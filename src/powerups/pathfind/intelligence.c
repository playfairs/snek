...
static int is_powerup_target_type(PowerupType type) {
  return type == POWERUP_PATHFIND || type == POWERUP_DOUBLE_POINTS ||
         type == POWERUP_FRENZY;
}
...
static int is_safe_move(GameContext *game, int new_dx, int new_dy) {
  ...
  int invincible = has_active_powerup(game, POWERUP_INVINCIBLE) ||
...
static int select_target(GameContext *game, Position *target_pos) {
  ...
  if (game->powerup.active && is_powerup_target_type(game->powerup.type)) {
    ...
    int head_distance = grid_distance_sq(
        game->snake.segments[game->snake.length - 1], powerup_pos);
    int food_distance = grid_distance_sq(
        game->snake.segments[game->snake.length - 1], food_pos);
...
void pathfind_update_intelligence(GameContext *game) {
  ...
  int directions[4][2] = {
      {0, -SNAKE_BLOCK}, {0, SNAKE_BLOCK}, {-SNAKE_BLOCK, 0}, {SNAKE_BLOCK, 0}};
