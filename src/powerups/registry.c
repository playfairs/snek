...
void spawn_powerup(Powerup *powerup) {
  ...
  PowerupType allowed[] = {
      POWERUP_SPEED_BOOST, POWERUP_SLOW_MO,  POWERUP_DOUBLE_POINTS,
      POWERUP_INVINCIBLE,  POWERUP_PATHFIND, POWERUP_FRENZY,
      POWERUP_MAGNET,      POWERUP_FREEZE,   POWERUP_SHRINK,
      POWERUP_GROW,        POWERUP_TELEPORT, POWERUP_SPLIT};
...
void update_powerup(Powerup *powerup, double current_time) {
  if (powerup->active &&
      current_time - powerup->spawn_time > POWERUP_ITEM_LIFETIME) {
    powerup->active = 0;
  }
}
...
void apply_powerup(GameContext *game, PowerupType type) {
  ...
  case POWERUP_SPEED_BOOST:
    duration = POWERUP_DURATION_SPEED_BOOST;
    break;
  case POWERUP_SLOW_MO:
    duration = POWERUP_DURATION_SLOW_MO;
    break;
  case POWERUP_DOUBLE_POINTS:
    duration = POWERUP_DURATION_DOUBLE_POINTS;
    break;
  case POWERUP_INVINCIBLE:
    duration = POWERUP_DURATION_INVINCIBLE;
    break;
  case POWERUP_PATHFIND:
    duration = POWERUP_DURATION_PATHFIND;
    break;
  case POWERUP_FRENZY:
    duration = POWERUP_DURATION_FRENZY;
    break;
  case POWERUP_MAGNET:
    duration = POWERUP_DURATION_MAGNET;
    break;
  case POWERUP_FREEZE:
    duration = POWERUP_DURATION_FREEZE;
    break;
  case POWERUP_SHRINK:
    duration = POWERUP_DURATION_SHRINK;
    break;
  case POWERUP_GROW:
    duration = POWERUP_DURATION_GROW;
    break;
  case POWERUP_TELEPORT:
    duration = POWERUP_DURATION_TELEPORT;
    break;
  case POWERUP_SPLIT:
    duration = POWERUP_DURATION_SPLIT;
    break;
  default:
  ...
  if (duration <= 0.0) {
    ...
    if (info.name) {
      snprintf(game->last_powerup_text, sizeof(game->last_powerup_text),
               "%s PICKED UP", info.name);
  ...
  if (game->powerup_count < MAX_POWERUPS) {
    ...
    game->active_powerups[game->powerup_count].end_time =
...
PowerupInfo get_powerup_info(PowerupType type) {
  switch (type) {
  case POWERUP_SPEED_BOOST:
    return (PowerupInfo){POWERUP_SPEED_BOOST, "SPEED BOOST",
                         LIGHT_BLUE_R,        LIGHT_BLUE_G,
                         LIGHT_BLUE_B,        POWERUP_DURATION_SPEED_BOOST};
  case POWERUP_SLOW_MO:
    return (PowerupInfo){POWERUP_SLOW_MO,         "SLOW MOTION", 145, 80, 255,
                         POWERUP_DURATION_SLOW_MO};
  case POWERUP_DOUBLE_POINTS:
    return (PowerupInfo){
        POWERUP_DOUBLE_POINTS,         "2X POINTS", 255, 215, 70,
        POWERUP_DURATION_DOUBLE_POINTS};
  case POWERUP_INVINCIBLE:
    return (PowerupInfo){POWERUP_INVINCIBLE,         "INVINCIBLE", 255, 80, 180,
                         POWERUP_DURATION_INVINCIBLE};
  case POWERUP_PATHFIND:
    return (PowerupInfo){POWERUP_PATHFIND,         "PATHFIND", 130, 255, 120,
                         POWERUP_DURATION_PATHFIND};
  case POWERUP_FRENZY:
    return (PowerupInfo){POWERUP_FRENZY,         "FRENZY", 255, 140, 35,
                         POWERUP_DURATION_FRENZY};
  case POWERUP_MAGNET:
    return (PowerupInfo){POWERUP_MAGNET,         "MAGNET", 220, 45, 45,
                         POWERUP_DURATION_MAGNET};
  case POWERUP_FREEZE:
    return (PowerupInfo){POWERUP_FREEZE,         "FREEZE", 155, 220, 255,
                         POWERUP_DURATION_FREEZE};
  case POWERUP_SHRINK:
    return (PowerupInfo){POWERUP_SHRINK,         "SHRINK", 128, 0, 32,
                         POWERUP_DURATION_SHRINK};
  case POWERUP_GROW:
    return (PowerupInfo){POWERUP_GROW, "GROW", 240,
                         230,          130,    POWERUP_DURATION_GROW};
  case POWERUP_TELEPORT:
    return (PowerupInfo){POWERUP_TELEPORT,         "TELEPORT", 170, 220, 255,
                         POWERUP_DURATION_TELEPORT};
  case POWERUP_SPLIT:
    return (PowerupInfo){POWERUP_SPLIT,         "SPLIT", 160, 0, 35,
                         POWERUP_DURATION_SPLIT};
  default:
