...
void spawn_powerup(Powerup *powerup)
{
    ...
    powerup->pos.y =
        GAME_AREA_TOP + (rand() % max_y) * SNAKE_BLOCK;

    PowerupType allowed[] = {POWERUP_SPEED_BOOST,
                             POWERUP_SLOW_MO,
                             ...
                             POWERUP_SPLIT};
    int choice =
        rand() % (sizeof(allowed) / sizeof(allowed[0]));
...
{
    if (powerup->active
        && current_time - powerup->spawn_time
               > POWERUP_ITEM_LIFETIME)
    {
        powerup->active = 0;
    }
}
...
void apply_powerup(GameContext *game, PowerupType type)
{
    ...
    if (duration <= 0.0)
    {
        ...
        if (info.name)
        {
            snprintf(game->last_powerup_text,
                     sizeof(game->last_powerup_text),
                     "%s PICKED UP",
                     info.name);
    ...
    for (int i = 0; i < game->powerup_count; i++)
    {
        ...
        {
            game->active_powerups[i].end_time =
    ...
    {
        game->active_powerups[game->powerup_count].type =
            type;
        game->active_powerups[game->powerup_count]
...
void update_active_powerup(GameContext *game,
                           double current_time)
{
    int write_index = 0;
    for (int i = 0; i < game->powerup_count; i++)
    {
        if (current_time
            > game->active_powerups[i].end_time)
        {
            remove_powerup_effect(
                game,
                game->active_powerups[i].type);
        }
        else
        {
            game->active_powerups[write_index++] =
...
PowerupInfo get_powerup_info(PowerupType type)
{
    ...
    {
    case POWERUP_SPEED_BOOST:
        return (PowerupInfo){POWERUP_SPEED_BOOST,
                             "SPEED BOOST",
                             LIGHT_BLUE_R,
                             LIGHT_BLUE_G,
                             LIGHT_BLUE_B,
                             POWERUP_DURATION_SPEED_BOOST};
    case POWERUP_SLOW_MO:
        return (PowerupInfo){POWERUP_SLOW_MO,
                             "SLOW MOTION",
                             145,
                             80,
                             255,
    ...
    case POWERUP_INVINCIBLE:
        return (PowerupInfo){POWERUP_INVINCIBLE,
                             "INVINCIBLE",
                             255,
                             80,
                             180,
    ...
    case POWERUP_PATHFIND:
        return (PowerupInfo){POWERUP_PATHFIND,
                             "PATHFIND",
                             130,
                             255,
                             120,
    ...
    case POWERUP_FRENZY:
        return (PowerupInfo){POWERUP_FRENZY,
                             "FRENZY",
                             255,
                             140,
                             35,
    ...
    case POWERUP_MAGNET:
        return (PowerupInfo){POWERUP_MAGNET,
                             "MAGNET",
                             220,
                             45,
                             45,
    ...
    case POWERUP_FREEZE:
        return (PowerupInfo){POWERUP_FREEZE,
                             "FREEZE",
                             155,
                             220,
                             255,
    ...
    case POWERUP_SHRINK:
        return (PowerupInfo){POWERUP_SHRINK,
                             "SHRINK",
                             128,
                             0,
                             32,
    ...
    case POWERUP_GROW:
        return (PowerupInfo){POWERUP_GROW,
                             "GROW",
                             240,
                             230,
                             130,
                             POWERUP_DURATION_GROW};
    case POWERUP_TELEPORT:
        return (PowerupInfo){POWERUP_TELEPORT,
                             "TELEPORT",
                             170,
                             220,
                             255,
    ...
    case POWERUP_SPLIT:
        return (PowerupInfo){POWERUP_SPLIT,
                             "SPLIT",
                             160,
                             0,
                             35,
    ...
    default:
        return (PowerupInfo){POWERUP_NONE,
                             "NONE",
                             WHITE_R,
                             WHITE_G,
                             WHITE_B,
...
void apply_powerup_effect(GameContext *game,
                          PowerupType type)
{
    switch (type)
    {
...
void update_powerup_effect(GameContext *game,
                           PowerupType type)
{
    switch (type)
    {
...
void remove_powerup_effect(GameContext *game,
                           PowerupType type)
{
    switch (type)
    {
