  ...
  for (int attempts = 0; attempts < 100; attempts++) {
    ...
    for (int di = 0; di < 4; di++) {
      ...
      for (int i = 0; i < game->snake.length; i++) {
        Position candidate = {head.x - (game->snake.length - 1 - i) * dir.x,
                              head.y - (game->snake.length - 1 - i) * dir.y};
        if (!is_inside_game(candidate.x, candidate.y) ||
      ...
      for (int i = 0; i < game->snake.length; i++) {
        game->snake.segments[i] =
            (Position){head.x - (game->snake.length - 1 - i) * dir.x,
                       head.y - (game->snake.length - 1 - i) * dir.y};
      }
      return;
    }
...
void teleport_update(GameContext *game) { (void)game; }

void teleport_remove(GameContext *game) { (void)game; }
