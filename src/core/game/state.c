...
static void parse_stats_line(const char *raw_line,
                             GameStats *stats)
{
    char line[256];
    ...
    else if (sscanf(trimmed, "total_apples = %d", &value)
             == 1)
    {
        stats->total_apples = value;
    }
    else if (sscanf(trimmed, "games_played = %d", &value)
             == 1)
    {
        stats->games_played = value;
    }
    else if (sscanf(trimmed,
                    "powerups_collected = %d",
                    &value)
             == 1)
    {
...
static int load_stats_file(const char *path,
                           GameStats *stats)
{
    char line[256];
...
static const char *get_xdg_config_dir(char *buffer,
                                      size_t size)
{
    ...
    {
        int n =
            snprintf(buffer, size, "%s/snek", config_home);
...
static const char *get_home_config_dir(char *buffer,
                                       size_t size)
{
    ...
    {
        int n =
            snprintf(buffer, size, "%s/.config/snek", home);
        if (n >= 0 && (size_t)n < size)
        {
            return buffer;
        }
...
int load_stats(GameStats *stats)
{
    ...
    const char *stats_dir =
        get_xdg_config_dir(config_dir, sizeof(config_dir));
    if (stats_dir)
    {
        int n = snprintf(user_path,
                         sizeof(user_path),
                         "%s/stats.toml",
                         stats_dir);
    ...
    stats_dir =
        get_home_config_dir(config_dir, sizeof(config_dir));
    if (stats_dir)
    {
        int n = snprintf(user_path,
                         sizeof(user_path),
...
void save_stats(GameStats *stats)
{
    ...
    char path[PATH_MAX];
    const char *stats_dir =
        get_home_config_dir(dir, sizeof(dir));
    if (stats_dir)
    {
        struct stat st = {0};
    ...
    if (!file)
    {
        fprintf(
            stderr,
            "save_stats: failed to open '%s' for writing\n",
            path);
        return;
    }
...
int load_settings(GameSettings *settings)
{
    ...
    while (fgets(line, sizeof(line), file))
    {
                 ...
                 == 0)
        {
            sscanf(line, "volume = %lf", &settings->volume);
                         ...
                         "current_skin",
                         sizeof("current_skin") - 1)
                 == 0)
        {
            sscanf(line,
                   "current_skin = %d",
                   (int *)&settings->current_skin);
        }
        else if (strncmp(line,
                         "difficulty",
                         sizeof("difficulty") - 1)
        ...
        else if (strncmp(line, "mode", sizeof("mode") - 1)
                 == 0)
        {
            sscanf(line,
                   "mode = %d",
    ...
    if (settings->current_skin < 0
        || settings->current_skin >= SKIN_COUNT)
    {
        settings->current_skin = SKIN_DEFAULT;
    }
    if (settings->difficulty < 0
        || settings->difficulty >= DIFFICULTY_COUNT)
    {
