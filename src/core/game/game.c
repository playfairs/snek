...
void init_game(GameContext *game, GameState *state) {
  ...
  int start_y =
      ((DIS_HEIGHT - GAME_AREA_TOP) / (2 * SNAKE_BLOCK)) * SNAKE_BLOCK +
      GAME_AREA_TOP;
  ...
  game->powerup_spawn_time = current_time + POWERUP_INITIAL_SPAWN_DELAY +
  ...
  game->mode_end_time = (game->mode == MODE_TIME_ATTACK)
                            ? current_time + TIME_ATTACK_DURATION
                            : 0;

  double spawn_delay =
      POWERUP_INITIAL_SPAWN_DELAY + rand() % POWERUP_SPAWN_DELAY_VARIANCE;
...
GameStatus game_loop(GameContext *game, GameState *state, AudioState *audio) {
  ...
  while (running && game->status == GAME_RUNNING) {
    ...
    while (SDL_PollEvent(&event)) {
      InputAction action =
          handle_input_snake(&event, game->snake.dx, game->snake.dy);
    ...
    if (!paused) {
      ...
      if (game->powerup.active &&
          current_time - game->powerup.spawn_time > POWERUP_ITEM_LIFETIME) {
        game->powerup.active = 0;
      }

      if (game->secondary_food.active &&
          current_time - game->secondary_food.spawn_time >
      ...
      if (game->food.active &&
          check_food_collision(&game->snake, &game->food)) {
        ItemType item_type = game->food.type;
        game->food.active = 0;
        int previous_tail_index =
            game->snake.length > 0 ? game->snake.length - 1 : 0;
        game->snake.length++;
        game->snake.segments[game->snake.length - 1] =
            game->snake.segments[previous_tail_index];
      ...
      if (game->powerup.active &&
          check_powerup_collision(&game->snake, &game->powerup)) {
    ...
    int glow_active = has_active_powerup(game, POWERUP_INVINCIBLE) ||
                      has_active_powerup(game, POWERUP_PATHFIND);
    ...
    draw_hud(state->renderer, state->score_font, state->button_font,
             ...
             game->last_powerup_text, game->last_powerup_display_time,
             current_time);
    ...
    if (paused) {
      ...
      SDL_Surface *paused_surf =
          TTF_RenderText_Blended(state->large_font, "PAUSED", white);
      SDL_Texture *paused_tex =
          SDL_CreateTextureFromSurface(state->renderer, paused_surf);
      SDL_Rect paused_rect = {(DIS_WIDTH - paused_surf->w) / 2,
                              GAME_AREA_TOP + GAME_AREA_HEIGHT / 2 - 40,
                              paused_surf->w, paused_surf->h};
...
void cleanup_game(GameContext *game) { (void)game; }
