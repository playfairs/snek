...
static int has_active_powerup(GameContext *game,
                              PowerupType type)
{
...
void init_game(GameContext *game, GameState *state)
{
    ...
    int start_x =
        (DIS_WIDTH / (2 * SNAKE_BLOCK)) * SNAKE_BLOCK;
    int start_y =
        ((DIS_HEIGHT - GAME_AREA_TOP) / (2 * SNAKE_BLOCK))
            * SNAKE_BLOCK
        + GAME_AREA_TOP;
    ...
    game->powerup_spawn_time =
        current_time + POWERUP_INITIAL_SPAWN_DELAY
        + rand() % POWERUP_SPAWN_DELAY_VARIANCE;
    ...
    game->mode_end_time =
        (game->mode == MODE_TIME_ATTACK)
            ? current_time + TIME_ATTACK_DURATION
            : 0;

    double spawn_delay =
        POWERUP_INITIAL_SPAWN_DELAY
        + rand() % POWERUP_SPAWN_DELAY_VARIANCE;
    ...
    game->powerup_spawn_time =
        game->start_time + spawn_delay;

    game->base_speed =
        get_base_speed(state->settings.difficulty)
        + (game->mode == MODE_CHALLENGE ? 2 : 0);
    ...
    while (running && game->status == GAME_RUNNING)
    {
            ...
            else if (action == INPUT_PAUSE)
            {
                ...
                else
                {
                    ...
                    double pause_duration =
                        resume_time - pause_start_time;
                    for (int i = 0; i < game->powerup_count;
                         i++)
                    {
        ...
        if (!paused)
        {
            if (!game->powerup.active
                && current_time > game->powerup_spawn_time)
            {
            ...
            if (game->powerup.active
                && current_time - game->powerup.spawn_time
            ...
            if (game->secondary_food.active
                && current_time
                           - game->secondary_food.spawn_time
                       > game->secondary_food.lifetime)
            {
                game->secondary_food.active = 0;
            }
            ...
            if (game->mode == MODE_TIME_ATTACK
                && current_time >= game->mode_end_time)
            {
            ...
            for (int i = 0; i < game->powerup_count; i++)
            {
                PowerupType type =
                    game->active_powerups[i].type;
                if (type == POWERUP_INVINCIBLE
                    || type == POWERUP_PATHFIND)
                {
                    bypass_collision = 1;
                    break;
                }
            ...
            frozen =
                has_active_powerup(game, POWERUP_FREEZE);
            game->frozen = frozen;
            if (!frozen)
            {
            ...
            if (!bypass_collision
                && check_self_collision(&game->snake))
            {
                game->status = GAME_OVER;
            }
                                        ...
                                        &game->food))
            {
                ...
                game->food.active = 0;
                int previous_tail_index =
                    game->snake.length > 0
                        ? game->snake.length - 1
                        : 0;
                game->snake.length++;
                game->snake
                    .segments[game->snake.length - 1] =
                    game->snake
                        .segments[previous_tail_index];

                game->apples_eaten++;
                state->stats.total_apples++;

                int points =
                    (item_type == ITEM_BANANA) ? 15 : 10;
                for (int i = 0; i < game->powerup_count;
                     i++)
                {
                    if (game->active_powerups[i].type
                ...
                spawn_primary_food(game);
                     ...
                     && check_food_collision(
                         ...
                         &game->secondary_food))
            {
                if (has_active_powerup(game,
                                       POWERUP_INVINCIBLE)
                    || has_active_powerup(game,
                                          POWERUP_PATHFIND))
                {
                    game->secondary_food.active = 0;
                }
            ...
            if (game->powerup.active
                && check_powerup_collision(&game->snake,
                                           &game->powerup))
            {
        ...
        int glow_active =
            has_active_powerup(game, POWERUP_INVINCIBLE)
            || has_active_powerup(game, POWERUP_PATHFIND);
        draw_snake(state->renderer,
                   &game->snake,
                   glow_active,
                   game->frozen);
