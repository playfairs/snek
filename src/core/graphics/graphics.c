...
static Color get_skin_color(SnakeSkin skin, int is_head) {
  ...
  case SKIN_RAINBOW:
    return is_head ? (Color){ACCENT_R, ACCENT_G, ACCENT_B}
                   : (Color){ACCENT2_R, ACCENT2_G, ACCENT2_B};
  case SKIN_PURPLE:
...
static Color get_skin_preview_color(SnakeSkin skin, int index, int is_head) {
  ...
  case SKIN_HOLOGRAPHIC: {
    ...
    int r = clamp_color(
        (int)(180 + 30 * sin(phase + 0.5) + 10 * sin(2 * phase + 0.8)));
    int g = clamp_color(
        (int)(190 + 28 * sin(phase + 1.2) + 8 * sin(2 * phase + 0.6)));
    int b = clamp_color(
        (int)(220 + 24 * sin(phase + 2.6) + 6 * sin(2 * phase + 1.9)));
    return (Color){r, g, b};
  }
  case SKIN_MONOCHROME: {
  ...
  case SKIN_SOLAR: {
    ...
    return (Color){
        base.r, base.g + (int)flare > 255 ? 255 : base.g + (int)flare, base.b};
  }
  case SKIN_AQUA: {
...
static Color get_powerup_color(PowerupType type) {
  ...
  case POWERUP_SLOW_MO:
    return (Color){145, 80, 255};
  case POWERUP_DOUBLE_POINTS:
    return (Color){255, 215, 70};
  case POWERUP_INVINCIBLE:
    return (Color){255, 80, 180};
  case POWERUP_PATHFIND:
    return (Color){130, 255, 120};
  case POWERUP_FRENZY:
    return (Color){255, 140, 35};
  case POWERUP_MAGNET:
    return (Color){220, 45, 45};
  case POWERUP_FREEZE:
    return (Color){155, 220, 255};
  case POWERUP_SHRINK:
    return (Color){128, 0, 32};
  case POWERUP_GROW:
    return (Color){240, 230, 130};
  case POWERUP_TELEPORT:
    return (Color){170, 220, 255};
  case POWERUP_SPLIT:
    return (Color){160, 0, 35};
  default:
    return (Color){WHITE_R, WHITE_G, WHITE_B};
  }
}

static SDL_Rect draw_text(SDL_Renderer *renderer, TTF_Font *font,
...
static SDL_Rect draw_text_centered(SDL_Renderer *renderer, TTF_Font *font,
...
static void draw_panel(SDL_Renderer *renderer, SDL_Rect rect, Color fill,
                       Color border) {
...
static Color apply_powerup_glow(Color base, int glow_active) {
  ...
  return (Color){base.r + highlight > 255 ? 255 : base.r + highlight,
                 base.g + highlight > 255 ? 255 : base.g + highlight,
                 base.b + highlight > 255 ? 255 : base.b + highlight};
}

void draw_cube(SDL_Renderer *renderer, int x, int y, int size, Color color,
               int is_head) {
  ...
  Color highlight = {
      ...
      color.b + (is_head ? 40 : 30) > 255 ? 255
                                          : color.b + (is_head ? 40 : 30)};
  ...
  SDL_RenderDrawLine(renderer, rect.x + 1, rect.y + rect.h - 1,
                     rect.x + rect.w - 1, rect.y + rect.h - 1);
  SDL_RenderDrawLine(renderer, rect.x + rect.w - 1, rect.y + 1,
                     rect.x + rect.w - 1, rect.y + rect.h - 1);

  if (is_head) {
    Color inner_highlight = {color.r + 40 > 255 ? 255 : color.r + 40,
                             color.g + 40 > 255 ? 255 : color.g + 40,
                             color.b + 40 > 255 ? 255 : color.b + 40};
    SDL_Rect inner_rect = {rect.x + 3, rect.y + 3, size / 3, size / 3};
    SDL_SetRenderDrawColor(renderer, inner_highlight.r, inner_highlight.g,
                           inner_highlight.b, 255);
...
void draw_grid(SDL_Renderer *renderer) {
  ...
  SDL_RenderDrawLine(renderer, 0, GAME_AREA_BOTTOM - 1, DIS_WIDTH - 1,
                     GAME_AREA_BOTTOM - 1);
}
...
void draw_powerup(SDL_Renderer *renderer, Powerup *powerup) {
  ...
  Color glow_color = {color.r + 50 > 255 ? 255 : color.r + 50,
  ...
  SDL_Rect glow_rect = {powerup->pos.x - glow_offset,
                        powerup->pos.y - glow_offset, glow_size, glow_size};
  SDL_SetRenderDrawColor(renderer, glow_color.r, glow_color.g, glow_color.b,
                         255);
  SDL_RenderDrawRect(renderer, &glow_rect);

  SDL_Rect powerup_rect = {powerup->pos.x, powerup->pos.y, SNAKE_BLOCK,
                           SNAKE_BLOCK};
  SDL_SetRenderDrawColor(renderer, color.r, color.g, color.b, 255);
...
void draw_hud(SDL_Renderer *renderer, TTF_Font *score_font,
              TTF_Font *button_font, int score, int high_score,
              int snake_length, int apples_collected, int current_speed,
              const ActivePowerup active_powerups[], int active_powerup_count,
              GameMode mode, double time_left, const char *last_powerup_text,
              double last_powerup_display_time, double current_time) {
  ...
  SDL_Surface *score_label_surf =
      TTF_RenderText_Blended(button_font, score_label, muted);
  SDL_Texture *score_label_tex =
      SDL_CreateTextureFromSurface(renderer, score_label_surf);
  SDL_Rect score_label_rect = {28, 20, score_label_surf->w,
                               score_label_surf->h};
  ...
  SDL_Surface *score_value_surf =
      TTF_RenderText_Blended(score_font, score_value, accent);
  SDL_Texture *score_value_tex =
      SDL_CreateTextureFromSurface(renderer, score_value_surf);
  SDL_Rect score_value_rect = {28, 32 + score_label_rect.h - 4,
                               score_value_surf->w, score_value_surf->h};
  ...
  SDL_Surface *high_label_surf =
      TTF_RenderText_Blended(button_font, high_label, muted);
  SDL_Texture *high_label_tex =
      SDL_CreateTextureFromSurface(renderer, high_label_surf);
  SDL_Rect high_label_rect = {DIS_WIDTH - high_label_surf->w - 28, 20,
                              high_label_surf->w, high_label_surf->h};
  ...
  SDL_Surface *high_value_surf =
      TTF_RenderText_Blended(score_font, high_value, white);
  SDL_Texture *high_value_tex =
      SDL_CreateTextureFromSurface(renderer, high_value_surf);
  SDL_Rect high_value_rect = {DIS_WIDTH - high_value_surf->w - 28,
                              32 + high_label_rect.h - 4, high_value_surf->w,
                              high_value_surf->h};
  ...
  const char *mode_name = mode == MODE_CHALLENGE     ? "CHALLENGE"
                          : mode == MODE_TIME_ATTACK ? "TIME ATTACK"
                                                     : "CLASSIC";
  char mode_text[24];
  ...
  SDL_Surface *mode_surf =
      TTF_RenderText_Blended(button_font, mode_text, white);
  SDL_Texture *mode_tex = SDL_CreateTextureFromSurface(renderer, mode_surf);
  SDL_Rect mode_rect = {mode_chip.x + 8, mode_chip.y + 4, mode_surf->w,
                        mode_surf->h};
  ...
  SDL_Surface *speed_surf =
      TTF_RenderText_Blended(button_font, speed_text, white);
  SDL_Texture *speed_tex = SDL_CreateTextureFromSurface(renderer, speed_surf);
  SDL_Rect speed_rect = {speed_chip.x + 8, speed_chip.y + 4, speed_surf->w,
                         speed_surf->h};
  ...
  SDL_Surface *apples_surf =
      TTF_RenderText_Blended(button_font, apples_text, white);
  SDL_Texture *apples_tex = SDL_CreateTextureFromSurface(renderer, apples_surf);
  SDL_Rect apples_rect = {apples_chip.x + 8, apples_chip.y + 4, apples_surf->w,
                          apples_surf->h};
  ...
  SDL_Rect len_rect = {len_chip.x + 8, len_chip.y + 4, len_surf->w,
                       len_surf->h};
  ...
  if (mode == MODE_TIME_ATTACK) {
    int bar_width = 220;
    int bar_height = 12;
    SDL_Rect time_bg = {DIS_WIDTH - bar_width - 28,
                        HEADER_HEIGHT - bar_height - 14, bar_width, bar_height};
    ...
    if (progress > 1.0)
      progress = 1.0;
    SDL_Rect time_fg = {time_bg.x, time_bg.y, (int)(bar_width * progress),
                        bar_height};
    ...
    SDL_Surface *timer_surf =
        TTF_RenderText_Blended(button_font, timer_text, white);
    SDL_Texture *timer_tex = SDL_CreateTextureFromSurface(renderer, timer_surf);
    SDL_Rect timer_rect = {time_bg.x + (bar_width - timer_surf->w) / 2,
                           time_bg.y - timer_surf->h - 4, timer_surf->w,
                           timer_surf->h};
  ...
  if (last_powerup_text && last_powerup_text[0] &&
      current_time - last_powerup_display_time < 2.5) {
    ...
    SDL_Surface *pickup_surf =
        TTF_RenderText_Blended(button_font, last_powerup_text, accent);
    if (pickup_surf) {
      SDL_Texture *pickup_tex =
          SDL_CreateTextureFromSurface(renderer, pickup_surf);
      SDL_Rect pickup_text_rect = {
          pickup_rect.x + (pickup_rect.w - pickup_surf->w) / 2,
          pickup_rect.y + (pickup_rect.h - pickup_surf->h) / 2, pickup_surf->w,
          pickup_surf->h};
  ...
  if (active_powerup_count > 0) {
    if (active_powerup_count > MAX_POWERUPS)
      active_powerup_count = MAX_POWERUPS;
    int chip_width = 160;
    int chip_height = 22;
    int chip_spacing = 8;
    ...
    for (int i = 0; i < active_powerup_count; i++) {
      ...
      double powerup_time_left =
          active_powerups[i].end_time - SDL_GetTicks() / 1000.0;
      if (powerup_time_left < 0.0)
        powerup_time_left = 0.0;
      char power_text[64];
      snprintf(power_text, sizeof(power_text), "%s %.1fs", name,
               powerup_time_left);
      ...
      SDL_Surface *power_surf =
          TTF_RenderText_Blended(button_font, power_text, accent);
      SDL_Texture *power_tex =
          SDL_CreateTextureFromSurface(renderer, power_surf);
      SDL_Rect power_rect = {power_chip.x + 10, power_chip.y + 3, power_surf->w,
                             power_surf->h};
...
void draw_main_menu(SDL_Renderer *renderer, TTF_Font *large_font,
                    TTF_Font *button_font, SnakeSkin current_skin) {
  ...
  draw_text(renderer, button_font, "Fast, clean, bite-sized arcade survival.",
            muted, title_area.x, title_area.y + 52);
  ...
  for (int i = 0; i < 8; i++) {
    preview_snake.segments[i].x =
        preview_x + preview_snake_col[i] * SNAKE_BLOCK;
    preview_snake.segments[i].y =
        preview_y + preview_snake_row[i] * SNAKE_BLOCK;
  }

  draw_snake(renderer, &preview_snake, 0, 0);

  Powerup powerup_preview = {{preview_x + preview_powerup_col * SNAKE_BLOCK,
                              preview_y + preview_powerup_row * SNAKE_BLOCK},
                             POWERUP_SPEED_BOOST,
                             1,
                             0.0};
  draw_powerup(renderer, &powerup_preview);

  Food bomb_preview = {{preview_x + preview_bomb_col * SNAKE_BLOCK,
                        preview_y + preview_bomb_row * SNAKE_BLOCK},
                       ITEM_BOMB,
                       1,
                       0.0,
                       0.0};
  ...
  Food apple_preview = {{preview_x + preview_apple_col * SNAKE_BLOCK,
                         preview_y + preview_apple_row * SNAKE_BLOCK},
                        ITEM_APPLE,
                        1,
                        0.0,
                        0.0};
  ...
  draw_text(renderer, button_font, "MAIN MENU", amber, action_panel.x + 32,
            action_panel.y + 32);
  draw_text(renderer, button_font, "WASD or arrows in game", muted,
            action_panel.x + 32, action_panel.y + 62);
  draw_text(renderer, button_font, "ESC pauses or backs out", muted,
            action_panel.x + 32, action_panel.y + 86);
}

void draw_settings_menu(SDL_Renderer *renderer, TTF_Font *large_font,
                        TTF_Font *score_font, TTF_Font *button_font,
                        GameState *state, int current_option) {
  (void)current_option;
  ...
  draw_text(renderer, large_font, "SETTINGS", accent, right_panel.x + 28,
            right_panel.y + 26);
  draw_text(renderer, button_font, "Click a row to cycle its value.", muted,
            right_panel.x + 30, right_panel.y + 78);
  draw_text(renderer, button_font, "Changes save immediately.", muted,
            right_panel.x + 30, right_panel.y + 102);

  const char *skin_names[] = {
      "Default",   "Blue",   "Red",     "Rainbow",     "Purple",
      "Gold",      "Cyan",   "Neon",    "Holographic", "Monochrome",
      "Chromatic", "Cosmic", "Chrome",  "Bubble",      "Shadow",
      "Solar",     "Aqua",   "Midnight"};
  ...
  draw_text(renderer, button_font, "SKIN PREVIEW", amber, left_panel.x + 24,
            left_panel.y + 28);
  draw_text(renderer, score_font, skin_names[skin_index], white,
            left_panel.x + 24, left_panel.y + 58);

  SDL_Rect preview_box = {left_panel.x + 24, left_panel.y + 112,
                          left_panel.w - 48, 190};
  ...
  for (int x = preview_box.x; x <= preview_box.x + preview_box.w; x += 20) {
    SDL_RenderDrawLine(renderer, x, preview_box.y, x,
                       preview_box.y + preview_box.h);
  }
  for (int y = preview_box.y; y <= preview_box.y + preview_box.h; y += 20) {
    SDL_RenderDrawLine(renderer, preview_box.x, y,
                       preview_box.x + preview_box.w, y);
  }
  ...
  for (int i = 0; i < 8; i++) {
    ...
    Color preview_color =
        get_skin_preview_color(state->settings.current_skin, i, i == 7);
    draw_cube(renderer, px, py, SNAKE_BLOCK, preview_color, i == 7);
  }

  SDL_Rect details = {left_panel.x + 24, left_panel.y + 330, left_panel.w - 48,
                      208};
  fill_rect(renderer, details, (Color){21, 30, 40});
  stroke_rect(renderer, details, (Color){52, 76, 89});
  draw_text(renderer, button_font, "CURRENT LOADOUT", amber, details.x + 18,
            details.y + 18);
  draw_text(renderer, button_font, mode_names[state->settings.mode], white,
            details.x + 18, details.y + 58);
  draw_text(renderer, button_font, difficulty_names[state->settings.difficulty],
            white, details.x + 18, details.y + 86);
  draw_text(renderer, button_font,
            state->settings.sound_enabled ? "Sound on" : "Sound off", muted,
            details.x + 18, details.y + 122);
  draw_text(renderer, button_font,
            state->settings.music_enabled ? "Music on" : "Music off", muted,
            details.x + 18, details.y + 148);
}

void draw_game_over(SDL_Renderer *renderer, TTF_Font *large_font,
                    TTF_Font *score_font, TTF_Font *button_font, int score) {
  (void)button_font;
  ...
  SDL_Surface *title_surf =
      TTF_RenderText_Blended(large_font, "GAME OVER", accent);
  SDL_Texture *title_tex = SDL_CreateTextureFromSurface(renderer, title_surf);
  SDL_Rect title_rect = {(DIS_WIDTH - title_surf->w) / 2, 220, title_surf->w,
                         title_surf->h};
  ...
  SDL_Surface *score_surf =
      TTF_RenderText_Blended(score_font, score_text, white);
  SDL_Texture *score_tex = SDL_CreateTextureFromSurface(renderer, score_surf);
  SDL_Rect score_rect = {(DIS_WIDTH - score_surf->w) / 2, 295, score_surf->w,
                         score_surf->h};
  SDL_RenderCopy(renderer, score_tex, NULL, &score_rect);
  SDL_FreeSurface(score_surf);
  SDL_DestroyTexture(score_tex);

  SDL_Surface *hint_surf = TTF_RenderText_Blended(
      button_font, "Hit PLAY AGAIN to try for a new high score.", gray);
  SDL_Texture *hint_tex = SDL_CreateTextureFromSurface(renderer, hint_surf);
  SDL_Rect hint_rect = {(DIS_WIDTH - hint_surf->w) / 2, 335, hint_surf->w,
                        hint_surf->h};
