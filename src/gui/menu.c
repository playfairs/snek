                              ...
                              const GuiRect *rect)
{
    ...
    for (int x = grid_start_x; x <= rect->x + rect->w;
         x += SNAKE_BLOCK)
    {
    ...
    for (int y = grid_start_y; y <= rect->y + rect->h;
         y += SNAKE_BLOCK)
    {
        SDL_RenderDrawLine(renderer,
                           rect->x,
                    ...
                    int *clicked)
{
    ...
    SDL_Color bg_color =
        is_pressed ? (SDL_Color){50, 85, 120, 255}
        : is_hover ? (SDL_Color){60, 100, 140, 255}
                   : (SDL_Color){28, 42, 55, 255};
    SDL_Color border_color =
        is_pressed ? (SDL_Color){90, 150, 200, 255}
    ...
    SDL_Surface *text_surf =
        TTF_RenderText_Blended(gui->button_font,
                               text,
                               white);
    if (text_surf)
    {
                               ...
                               const GameStats *stats)
{
    ...
    GuiRect shell =
        gui_make_rect(gui, 0.08f, 0.08f, 0.84f, 0.84f);
    draw_panel(renderer,
               &shell,
               (SDL_Color){16, 22, 31, 255},
               (SDL_Color){58, 83, 98, 255});

    draw_text(renderer,
              gui->large_font,
              "SNEK",
              (SDL_Color){124, 218, 184, 255},
              shell.x + 24,
              shell.y + 20);
    draw_text(renderer,
              gui->button_font,
              "Fast, clean, bite-sized arcade survival.",
              (SDL_Color){158, 170, 184, 255},
              shell.x + 24,
              shell.y + 56);
    ...
    GuiRect preview_panel = {left_col_x,
                             shell.y
                                 + (int)(shell.h * 0.15f),
                             left_col_w,
                             preview_h};
    draw_panel(renderer,
               &preview_panel,
               (SDL_Color){18, 26, 35, 255},
               (SDL_Color){58, 83, 98, 255});
    draw_text(renderer,
              gui->button_font,
              "CURRENT SKIN",
              (SDL_Color){255, 196, 92, 255},
              preview_panel.x + 16,
              preview_panel.y + 12);
    draw_preview_grid(renderer, &preview_panel);
    draw_snake_preview(renderer,
                       current_skin,
                       &preview_panel,
                       0);
    ...
    GuiRect stats_panel = {left_col_x,
                           preview_panel.y + preview_panel.h
                               + 12,
                           left_col_w,
                           stats_h};
    draw_panel(renderer,
    ...
    if (stats)
    {
        ...
        draw_text(
            ...
            gui->button_font,
            high_score_text,
            (SDL_Color){WHITE_R, WHITE_G, WHITE_B, 255},
            stats_panel.x + 16,
            stats_panel.y + 40);
        ...
        snprintf(powerups_text,
                 sizeof(powerups_text),
                 "Powerups: %d",
                 POWERUP_NONE);
        draw_text(
            renderer,
    ...
    int right_col_w =
        shell.w - left_col_w - (int)(shell.w * 0.06f) - 20;

    GuiRect action_panel = {right_col_x,
                            shell.y
                                + (int)(shell.h * 0.15f),
                            right_col_w,
                            shell.h - (int)(shell.h * 0.15f)
                                - (int)(shell.h * 0.05f)};
    draw_panel(renderer,
               &action_panel,
               (SDL_Color){20, 29, 39, 255},
               (SDL_Color){69, 92, 105, 255});
    draw_text(renderer,
              gui->button_font,
              "MAIN MENU",
              (SDL_Color){255, 196, 92, 255},
    ...
    const GameStats *stats)
{
    ...
    draw_panel(renderer,
               &frame,
               (SDL_Color){15, 21, 30, 255},
               (SDL_Color){57, 80, 95, 255});
    ...
    int right_col_x = left_col_x + left_col_w + 20;
    int right_col_w =
        frame.w - left_col_w - (int)(frame.w * 0.06f) - 20;

    GuiRect settings_panel = {
        right_col_x,
        frame.y + (int)(frame.h * 0.13f),
        right_col_w,
        frame.h - (int)(frame.h * 0.13f)
            - (int)(frame.h * 0.05f)};
    draw_panel(renderer,
               &settings_panel,
               (SDL_Color){20, 29, 39, 255},
                                ...
                                const GameStats *stats)
{
    ...
    GuiRect panel =
        gui_make_rect(gui, 0.08f, 0.11f, 0.84f, 0.74f);
    draw_panel(renderer,
               &panel,
               (SDL_Color){22, 28, 40, 255},
               (SDL_Color){80, 120, 170, 255});
    draw_text(renderer,
              gui->large_font,
              "STATISTICS",
...
void gui_menu_render_game_over(GuiContext *gui, int score)
{
    ...
    GuiRect card =
        gui_make_rect(gui, 0.18f, 0.21f, 0.64f, 0.42f);
    draw_panel(renderer,
               &card,
               (SDL_Color){24, 32, 46, 255},
