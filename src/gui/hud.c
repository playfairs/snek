...
void gui_hud_render(GuiContext *gui, const GuiHudData *data)
{
    ...
    GuiRect header =
        gui_make_rect(gui, 0.0f, 0.0f, 1.0f, 0.09f);
    SDL_SetRenderDrawColor(renderer, 12, 18, 28, 255);
    SDL_RenderFillRect(
        renderer,
        &(SDL_Rect){0, 0, gui->layout.width, header.h});

    GuiRect border =
        gui_make_rect(gui, 0.008f, 0.01f, 0.984f, 0.07f);
    draw_panel(renderer,
               &border,
               (SDL_Color){24, 32, 46, 255},
               (SDL_Color){60, 90, 130, 255});
    ...
    const char *mode_name =
        data->mode == MODE_CHALLENGE     ? "CHALLENGE"
        : data->mode == MODE_TIME_ATTACK ? "TIME ATTACK"
                                         : "CLASSIC";
    char mode_text[24];
    ...
    if (data->mode == MODE_TIME_ATTACK)
    {
        ...
        int bar_height = 12;
        SDL_Rect time_bg = {gui->layout.width - bar_width
                                - 28,
        ...
        double progress =
            data->time_left / TIME_ATTACK_DURATION;
        if (progress < 0.0)
        {
            progress = 0.0;
        }
    ...
    if (data->last_powerup_text[0]
        && data->current_time
                   - data->last_powerup_display_time
               < 2.5)
    {
        GuiRect pickup =
            gui_make_rect(gui, 0.28f, 0.11f, 0.44f, 0.035f);
        SDL_Rect pickup_rect = {pickup.x,
                                pickup.y,
                                pickup.w,
                                pickup.h};
    ...
    {
        int chip_width =
            gui_layout_scale(&gui->layout, 160);
        int chip_height = 22;
        int chip_spacing = 8;
        ...
        for (int i = 0; i < data->active_powerup_count; i++)
        {
            ...
            double powerup_time_left =
                data->active_powerups[i].end_time
                - data->current_time;
