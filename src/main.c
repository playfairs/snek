...
typedef enum { MENU_MAIN, MENU_SETTINGS, MENU_STATS, MENU_GAME_OVER } MenuState;

static void draw_stats_menu_local(SDL_Renderer *renderer, TTF_Font *large_font,
                                  TTF_Font *score_font, TTF_Font *button_font,
                                  GameState *state);

static int draw_button(SDL_Renderer *renderer, TTF_Font *font, const char *text,
                       int x, int y, int w, int h, int mouse_x, int mouse_y,
                       int *clicked) {
  ...
  SDL_Rect text_rect = {x + (w - text_surf->w) / 2, y + (h - text_surf->h) / 2,
...
static MenuState main_menu_loop(GameState *state, AudioState *audio) {
  ...
  while (running) {
    ...
    int play_clicked = draw_button(
        state->renderer, state->button_font, "PLAY GAME", start_x, button_y,
        button_width, button_height, mouse_x, mouse_y, &mouse_clicked);
    int settings_clicked =
        draw_button(state->renderer, state->button_font, "SETTINGS", start_x,
                    button_y + button_height + button_spacing, button_width,
                    button_height, mouse_x, mouse_y, &mouse_clicked);
    int stats_clicked = draw_button(
        state->renderer, state->button_font, "STATS", start_x,
        button_y + (button_height + button_spacing) * 2, button_width,
        button_height, mouse_x, mouse_y, &mouse_clicked);
    int quit_clicked = draw_button(
        state->renderer, state->button_font, "EXIT", start_x,
        button_y + (button_height + button_spacing) * 3, button_width,
        button_height, mouse_x, mouse_y, &mouse_clicked);
...
static MenuState settings_menu_loop(GameState *state, AudioState *audio) {
  ...
  while (running) {
    ...
    int skin_clicked = draw_button(
        state->renderer, state->button_font, skin_text, start_x, start_y,
        button_width, button_height, mouse_x, mouse_y, &mouse_clicked);
    if (skin_clicked) {
    ...
    char sound_text[64];
    snprintf(sound_text, sizeof(sound_text), "Sound: %s",
             sound_names[sound_index]);
    int sound_clicked =
        draw_button(state->renderer, state->button_font, sound_text, start_x,
                    start_y + (button_height + button_spacing), button_width,
                    button_height, mouse_x, mouse_y, &mouse_clicked);
    if (sound_clicked) {
    ...
    char music_text[64];
    snprintf(music_text, sizeof(music_text), "Music: %s",
             music_names[music_index]);
    int music_clicked = draw_button(
        state->renderer, state->button_font, music_text, start_x,
        start_y + (button_height + button_spacing) * 2, button_width,
        button_height, mouse_x, mouse_y, &mouse_clicked);
    if (music_clicked) {
    ...
    char volume_text[64];
    snprintf(volume_text, sizeof(volume_text), "Volume: %s",
             volume_names[volume_index]);
    int volume_clicked = draw_button(
        state->renderer, state->button_font, volume_text, start_x,
        start_y + (button_height + button_spacing) * 3, button_width,
        button_height, mouse_x, mouse_y, &mouse_clicked);
    if (volume_clicked) {
    ...
    int mode_clicked = draw_button(
        state->renderer, state->button_font, mode_text, start_x,
        start_y + (button_height + button_spacing) * 4, button_width,
        button_height, mouse_x, mouse_y, &mouse_clicked);
    if (mode_clicked) {
    ...
    int difficulty_clicked = draw_button(
        state->renderer, state->button_font, difficulty_text, start_x,
        start_y + (button_height + button_spacing) * 5, button_width,
        button_height, mouse_x, mouse_y, &mouse_clicked);
    ...
    int back_clicked = draw_button(
        state->renderer, state->button_font, "BACK", start_x,
        start_y + (button_height + button_spacing) * 6, button_width,
        button_height, mouse_x, mouse_y, &mouse_clicked);
    if (back_clicked) {
      running = 0;
    }
...
static MenuState stats_menu_loop(GameState *state) {
  ...
  while (running) {
    ...
    draw_stats_menu_local(state->renderer, state->large_font, state->score_font,
                          state->button_font, state);

    int back_clicked = draw_button(
        state->renderer, state->button_font, "BACK", start_x, start_y,
        button_width, button_height, mouse_x, mouse_y, &mouse_clicked);
...
static void draw_stats_menu_local(SDL_Renderer *renderer, TTF_Font *large_font,
  ...
  SDL_Surface *title_surf =
      TTF_RenderText_Blended(large_font, "STATISTICS", accent);
  SDL_Texture *title_tex = SDL_CreateTextureFromSurface(renderer, title_surf);
  SDL_Rect title_rect = {(DIS_WIDTH - title_surf->w) / 2, 120, title_surf->w,
                         title_surf->h};
  ...
  snprintf(buf, sizeof(buf), "Powerups Collected: %d",
...
static MenuState game_over_loop(GameState *state, int score) {
  ...
  while (running) {
    ...
    draw_game_over(state->renderer, state->large_font, state->score_font,
                   state->button_font, score);

    int total_width = button_width * 2 + button_spacing;
    int start_x = (DIS_WIDTH - total_width) / 2;

    int play_again_clicked = draw_button(
        state->renderer, state->button_font, "PLAY AGAIN", start_x, button_y,
        button_width, button_height, mouse_x, mouse_y, &mouse_clicked);
...
int main(int argc, char *argv[]) {
  ...
  state.window =
      SDL_CreateWindow("Snek", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
