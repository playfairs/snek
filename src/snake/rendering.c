...
static Color get_skin_effect_color(SnakeSkin skin, int index, int is_head) {
  ...
  if (skin == SKIN_GOLD) {
    ...
    return (Color){clamp_color(base_color.r + shine),
                   clamp_color(base_color.g + shine), base_color.b};
  }
...
void draw_snake(SDL_Renderer *renderer, const Snake *snake, int glow_active,
  ...
  for (int i = 0; i < snake->length - 1; i++) {
    ...
    if (snake->skin == SKIN_RAINBOW) {
      double time = SDL_GetTicks() / 200.0;
      double phase = (i * 0.3) + (snake->segments[i].x * 0.01) +
      ...
      base_color = (Color){
          base_color.r + glow > 255 ? 255 : base_color.r + glow, base_color.g,
      ...
      base_color =
          (Color){base_color.r + shine > 255 ? 255 : base_color.r + shine,
      ...
      base_color =
          (Color){base_color.r + glow > 255 ? 255 : base_color.r + glow,
      ...
      base_color = (Color){
          base_color.r, base_color.g + heat > 255 ? 255 : base_color.g + heat,
          base_color.b};
      ...
      base_color =
          (Color){base_color.r, base_color.g,
    ...
    draw_cube(renderer, snake->segments[i].x, snake->segments[i].y, SNAKE_BLOCK,
              color, 0);
  }
